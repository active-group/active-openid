...
* Publish a new release
  =active-openid= provides a =Makefile= with a =release= target.  To
  release a new version to [[https://clojars.org/][Clojars]], the =release= target relies on two
  ...
  #+begin_src 
    CLOJARS_USERNAME=<username> CLOJARS_PASSWORD=<cli-token> make release
  #+end_src
* TODO [1/3]
